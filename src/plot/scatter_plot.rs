        ...
        .show(ctx, |ui| {
            egui::ScrollArea::vertical().max_height(560.0).show(ui, |ui| {
                ...
                if self.draft_color_variant == 2 {
                    ...
                    ui.add_space(2.0);
                    colormap_combo(ui, "cfg_colormap", &mut self.draft_colormap, self.draft_color_reverse, theme);
...
fn colormap_combo(ui: &mut Ui, id: &str, colormap: &mut Colormap, reverse: bool, theme: &AppTheme) {
    ...
    // Selected text: name + inline swatch
    let selected_label = colormap.label().to_string();

    egui::ComboBox::from_id_salt(id)
        .selected_text(RichText::new(&selected_label).color(c.text_primary).size(s.font_body))
        .width(ui.available_width())
        .show_ui(ui, |ui| {
